...
| 이름 | 담당 역할 |
|---|---|
| 이시환 (본인) | 회원 인증 시스템(회원가입/로그인/아이디·비밀번호 찾기/탈퇴) 백엔드 개발(MySQL/TiDB Cloud) + YOLOv8 모델 학습(씽크홀 데이터셋 추가 파인튜닝) |
| 팀원 2 | 추후 보완 |
| 팀원 3 | 추후 보완 |
| 팀원 4 | 추후 보완 |
| 팀원 5 | 추후 보완 |

> 팀원별 상세 역할은 발표자료 확인 후 업데이트할 예정입니다.
...
**1. 회원 인증 시스템 백엔드**
- TiDB Cloud(MySQL 호환) 연동, `werkzeug.security`로 비밀번호 해싱 처리
- 회원가입: 이메일 형식 검증, 닉네임 비속어 필터링, 아이디/닉네임/이메일 중복 검사
- 회원가입 시 아이디·이메일 실시간 중복확인 API(AJAX)
- 아이디 찾기 / 비밀번호 찾기·재설정 API
- 로그인/로그아웃, 세션 기반 인증 및 관리자 권한 동기화
- 마이페이지 연동(회원정보 수정, 알림 설정, 탈퇴 처리)

**2. YOLOv8 모델 학습**
- 기존 AI-Hub 공공 도로손상 데이터셋(약 21.5만 장)으로 학습된 v5(YOLOv8s, 100 epoch) 모델을 베이스로 사용
- Roboflow에서 씽크홀(Sinkhole) 데이터셋을 추가 수집하고, 팀원들과 함께 씽크홀이 아닌 이미지를 육안으로 검수해 필터링
- v5 가중치 기반 전이학습(Transfer Learning)으로 씽크홀 클래스를 통합한 v6 모델을 18 epoch 추가 파인튜닝 (AdamW, batch 32)
- 18종 세부 클래스를 5종 핵심 클래스(Pothole, Major/Minor Crack, Asset, Sinkhole)로 재매핑·단순화
- 최종 모델(`best.pt`) Flask 추론 파이프라인에 연동
